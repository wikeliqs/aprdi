...
class Home extends CI_Controller {
    ...
    public function __construct()
    {
		...
		// print_r($this->session->userdata('is_verify'));
		if($this->session->userdata('user_login')){
			// print_r($this->session->userdata('is_verify'));
			if ($this->session->userdata('is_verify') == '') {
				// print_r(($this->session->userdata('kd_verify');
             redirect(site_url('login/confirmation'), 'refresh');
			}else{
		 ...
		 /* if ($this->session->userdata('kd_verify') != true) {
            redirect(site_url('login/confirmation'), 'refresh');
        } */
    ...
    public function instructor_page() {
    ...
    public function lesson($slug = "", $course_id = "", $lesson_id = "") {
        ...
        if (sizeof($page_data['sections']) > 0) {
            if ($lesson_id == "") {
                ...
                $video_details = $this->video_model->getVideoDetails($default_lesson_details['video_url']);
                $page_data['provider']   = $video_details['provider'];
                $page_data['video_id']   = $video_details['video_id'];
                $page_data['lesson_id']  = $default_lesson_details['id'];
            }else {
                $lesson_details = $this->crud_model->get_lessons('lesson', $lesson_id)->row_array();
                $video_details = $this->video_model->getVideoDetails($lesson_details['video_url']);
                $page_data['provider']   = $video_details['provider'];
                $page_data['video_id']   = $video_details['video_id'];
