...
class Admin extends CI_Controller {
    public function __construct()
    {
		...
		/* if($this->session->userdata('admin_login')){
			if ($this->session->userdata('is_verify') != true) {
    ...
    public function courses($default_category_id = "", $default_sub_category_id = "") {
        if ($this->session->userdata('admin_login') != true) {
            redirect(site_url('login'), 'refresh');
        }
        ...
        $page_data['categories'] = $this->crud_model->get_categories();
        $page_data['course_name'] = $this->crud_model->get_courses();
		 
		   // log_message('debug', var_export($page_data));
        if (isset($_POST['category_id']) && isset($_POST['sub_category_id'])) {
            ...
            redirect(site_url('admin/courses/'.$this->input->post('category_id').'/'.$this->input->post('sub_category_id')));
        }else {
        ...
        $page_data['page_title'] = get_phrase('courses');
		   // print_r($page_data['courses']->result());
        $this->load->view('backend/index', $page_data);
    }
